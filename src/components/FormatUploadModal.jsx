...
export default function FormatUploadModal({ show, onClose }) {
  ...
  const jsCodePlain = `export const tambahanSoal = [
  {
    id: 11,
    ...
    pembahasan: "CPU (Central Processing Unit) adalah komponen utama komputer yang menjalankan instruksi program. CPU terdiri dari ALU (Arithmetic Logic Unit) untuk operasi matematika dan logika, CU (Control Unit) untuk mengatur aliran data, dan Register untuk penyimpanan sementara."
  }
];`;
  ...
  const jsCodeLatex = `export const tambahanSoal = [
  {
    ...
    pembahasan: "Ini adalah limit fundamental trigonometri yang sangat penting.\\n\\nBukti menggunakan Aturan L'Hôpital:\\n$$\\\\lim_{x \\\\to 0} \\\\frac{\\\\sin x}{x} = \\\\lim_{x \\\\to 0} \\\\frac{\\\\cos x}{1} = \\\\cos(0) = 1$$\\n\\nNilai ini sering digunakan sebagai dasar dalam kalkulus."
  }
];`;
      ...
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl animate-scaleIn"
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="p-4 rounded-xl" style={{ background: '#0B1121', border: '1px solid rgba(0,229,255,0.12)' }}>
            ...
            <div className="space-y-1.5">
              {[
                ...
                { name: 'pembahasan',   wajib: false, desc: 'Penjelasan panjang step-by-step (bisa LaTeX $$...$$)' },
                { name: 'id',           wajib: false, desc: 'ID unik, akan di-generate otomatis jika tidak diisi' },
              ].map(f => (
                <div key={f.name} className="flex items-start gap-2 text-xs">
                  <code className="px-1.5 py-0.5 rounded flex-shrink-0 font-mono" style={{ background: '#1E293B', color: '#00E5FF' }}>{f.name}</code>
                  {f.wajib
                    ? <span className="px-1.5 py-0.5 rounded text-xs flex-shrink-0" style={{ background: 'rgba(239,68,68,0.12)', color: '#EF4444' }}>wajib</span>
                    : <span className="px-1.5 py-0.5 rounded text-xs flex-shrink-0" style={{ background: 'rgba(100,116,139,0.15)', color: '#64748B' }}>opsional</span>
                  }
          ...
          <div>
            ...
            <div className="mt-2 p-3 rounded-lg text-xs" style={{ background: 'rgba(168,85,247,0.06)', border: '1px solid rgba(168,85,247,0.15)' }}>
              <span className="font-semibold flex items-center gap-1" style={{ color: '#A855F7' }}>
                <i className="fa-solid fa-triangle-exclamation text-xs" /> Perhatian:
              </span>{' '}
              <span style={{ color: '#7C3AED' }}>Dalam file .js, backslash LaTeX harus ditulis double: <code style={{ color: '#A855F7' }}>\\\\frac</code> bukan <code style={{ color: '#A855F7' }}>\\frac</code>, karena string JavaScript mengkonversi \\ menjadi \.</span>
            </div>
          </div>
          ...
          <div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr style={{ background: '#0B1121' }}>
                      {['No','Soal','Pil. A','Pil. B','Pil. C','Pil. D','Jawaban','Penjelasan','Pembahasan'].map(h => (
            ...
            <p className="text-xs mt-2 flex items-start gap-1.5" style={{ color: '#475569' }}>
              ...
              LaTeX di DOCX cukup tulis normal: <code style={{ color: '#00E5FF' }}>$\frac{'{'x{'}'}{'{'}2{'}'}}$</code> (single backslash, tidak perlu di-escape).
            </p>
          </div>
        </div>
